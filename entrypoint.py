...
argument_parser.add_argument('--gh-server-url', type=str, default='https://github.com')
...
GITHUB_SERVER_URL = args.gh_server_url
DEFAULT_BRANCH = args.default_branch
RAW_GITHUB_CONTENT_URL = args.raw_gh_content_url
...
def fetch_files(root_dir, exclusions):
    ...
    for markdown_file in markdown_files:
        ...
        if root_dir and root_dir != '':
            url_list.append(
                f'{GITHUB_SERVER_URL}/{GITHUB_REPOSITORY}/blob/{DEFAULT_BRANCH}/{root_dir}/{url_encoded_path}')
        else:
            url_list.append(
                f'{GITHUB_SERVER_URL}/{GITHUB_REPOSITORY}/blob/{DEFAULT_BRANCH}/{url_encoded_path}')
...
def construct_described_by():
    return [
        SignPost(type='application/yaml', profile='https://citation-file-format.github.io/1.2.0/schema.json',
                 href=f'{RAW_GITHUB_CONTENT_URL}/{GITHUB_REPOSITORY}/{DEFAULT_BRANCH}/CITATION.cff')
    ]
